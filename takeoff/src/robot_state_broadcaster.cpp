...
RobotStateBroadcaster::RobotStateBroadcaster() {
    ...
    while (nh_.ok()) {
        ...
        if (!get_model_state_.call(gms_ardrone_))
            ROS_ERROR("RobotStateBroadcaster::58::Failed to call service ardrone");
        if (!gms_ardrone_.response.success)
            ROS_ERROR("RobotStateBroadcaster::58::Response to service ardrone failed");
        else {
            ...
            gt_altitude.gt_altitude = gms_ardrone_.response.pose.position.z;
            altitude_pub_.publish(gt_altitude);
        }

        // quadrotor
        // if (!gms_ardrone_client_.call(gms_ardrone_))
        //     ROS_ERROR("Failed to call service ardrone");
        // if (!gms_ardrone_.response.success)
        ...
        // summit_xl
        // if (!gms_summit_client_.call(gms_summit_))
